...
4. Use **Preserve details and shadows** and **Remove background spill** to adjust the one RGBA mask returned by RMBG-2.0. Both controls range from `0` to `100` and update the existing preview mask locally in the browser; changing them does not run RMBG-2.0 again. Preserve keeps more weak semi-transparent edges, particles, shadows, and small details. Remove background spill reduces the faint translucent halo around the subject. When both values are `0`, the original model mask is shown unchanged. Fully transparent pixels cannot be restored by Preserve because RMBG-2.0 returns one mask, not separate editable layers. Use **Cutout** to inspect the transparent foreground and **Mask** to view the adjusted alpha channel in grayscale, which makes subtle changes visible even on a white background. Use **Zoom** above the comparison to inspect the same area in Source and Removed background; scrolling either panel pans both panels together.
5. After the first run the action is named **Regenerate model mask**. It runs only RMBG-2.0 and is independent of both correction controls. If the model produces byte-for-byte the same mask, the editor reports that result; keep using the controls to adjust the existing mask instead of regenerating it.

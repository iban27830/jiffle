...
4. Use **Preserve details and shadows** to control the edge: higher values keep more particles, shadows, and soft translucent details; lower values remove more uncertain background pixels. This control updates the saved mask locally and does not run RMBG-2.0 again. Use **Zoom** above the comparison to inspect the same area in Source and Removed background; scrolling either panel pans both panels together.
5. Choose a background category and select an image from the local background library. To add a new background, enter its category, choose a local image file, and select **Import background**. You can change the background repeatedly after the foreground preview is ready.
6. Adjust **Blur**, select **Preview result**, and check the composition. Select **Apply background** only after the composition preview is ready. The apply action creates a new PNG version and activates it immediately; the previous version remains available under **Versions** and can be restored later.

...
4. Use **Preserve details and shadows** to control the edge: higher values keep more particles, shadows, and soft translucent details; lower values remove more uncertain background pixels. The mask changes immediately in the browser and does not run RMBG-2.0 again. After a successful preview, the same button becomes **Run removal again** when you explicitly want a fresh model result. If that retry fails, the previous working preview remains available. Use **Zoom** above the comparison to inspect the same area in Source and Removed background; scrolling either panel pans both panels together.

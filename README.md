...
2. In **Settings → Background removal**, leave **Removal model** set to **Automatic (recommended)**. Automatic uses the public BiRefNet model for finer edges and can fall back to RMBG-2.0 when its weights are cached or a valid token permits downloading them. A Hugging Face `Read` token is optional for BiRefNet but is required for the RMBG-2.0 fallback; paste it into **Hugging Face token** and save settings if you want that fallback available. Select **Test access** after saving a token to verify it. The token itself is never shown back by the settings API.
3. In the image editor, open **Replace background** and select **Remove background / Preview**. The first request may take several minutes while Jiffle installs the local segmentation runtime (including its PyTorch, `kornia`, and `timm` dependencies) and downloads the selected model weights. Network access is required only for this setup; later requests use the cached model. A completed foreground preview is kept for the active image version, so reopening the editor does not run the model again.
4. In the usual workflow, select a background and press **Apply background**; the automatic mask is used without any extra tuning. If an edge needs attention, open **Fine-tune mask edges**. **Preserve soft details** and **Clean edge spill** adjust the one RGBA mask locally in the browser and do not run the model again. They affect semi-transparent edge pixels only; they cannot recover a detail that the model removed completely or remove an opaque region that the model classified as foreground. Use **Cutout** to inspect the transparent foreground and **Mask** to view the adjusted alpha channel in grayscale. Use **Zoom** above the comparison to inspect the same area in Source and Removed background; scrolling either panel pans both panels together.
5. After the first run the action is named **Regenerate automatic mask**. It runs the selected automatic model again and is independent of the fine-tuning controls. If the model produces the same mask, the editor reports that result; the existing preview can still be applied.
6. Select **Choose background** to open the background library. Filter by category, or enter an import category and choose a local image file; imported backgrounds appear in the open modal and keep the selected category. Clicking a card only makes a temporary choice. Select **Select background** to confirm it, or use **Cancel**, **Escape**, or the backdrop to close the library without changing the current background.
7. Adjust **Blur** and check the automatically updated composition preview. Blur, Preserve, and Remove background spill use the same values in the local preview and when you select **Apply background**. The apply action sends the chosen values to the server, which creates and activates a new full-resolution PNG version; the previous version remains available under **Versions** and can be restored later.
...
If **Test access** reports an invalid token, create a new read token and replace the saved value. If it reports that access is denied, accept the RMBG-2.0 terms while logged in to Hugging Face. If it reports that Hugging Face is unavailable, check the network or proxy settings. Model setup also requires enough disk space for PyTorch and the model cache. A GPU is optional; CPU processing is supported but slower. If the preview reports that no usable subject was isolated, try another image or correct the source version before composing. If the editor reports that a preview is stale, restore the source version that produced it or run **Remove background / Preview** again.

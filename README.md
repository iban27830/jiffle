...
4. Use **Preserve details and shadows** to adjust the existing removal mask. The value is from `0` to `100`: `0` keeps the model mask unchanged, while higher values keep more semi-transparent edge pixels, particles, and soft shadows. The preview updates immediately in the browser; changing this value does not run RMBG-2.0 again. RMBG-2.0 returns one RGBA mask, not separate editable layers, so Preserve cannot restore pixels that the model already made fully transparent. If the editor reports that the mask has no semi-transparent edge pixels, use **Run removal again** with the same or another source version and check the new result. Use **Zoom** above the comparison to inspect the same area in Source and Removed background; scrolling either panel pans both panels together.
...
6. Adjust **Blur** and check the automatically updated composition preview. Both **Blur** and **Preserve details and shadows** update the preview locally without a network request for every slider event. Select **Apply background** when the composition is ready. The apply action sends the chosen values to the server, which creates and activates a new full-resolution PNG version; the previous version remains available under **Versions** and can be restored later.

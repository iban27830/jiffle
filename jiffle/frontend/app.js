...
function renderSetStatus(current) {
  const partial = current.result || {};
  const processed = current.processed ?? partial.processed ?? 0;
  const total = current.total ?? partial.total ?? '?';
  const name = current.result?.set?.name || current.set?.name || 'e621 set';
  const stopping = Boolean(current.cancel_requested);
  const phase = {fetch_set: 'fetching set', importing_posts: 'importing posts', finalizing: 'finalizing'}[partial.phase] || 'importing posts';
  const elapsed = partial.elapsed_ms != null ? ` · ${formatDuration(partial.elapsed_ms)}` : '';
  const active = Array.isArray(partial.active_post_ids) && partial.active_post_ids.length
    ? ` · active: ${partial.active_post_ids.map(id => `#${esc(id)}`).join(', ')}` : '';
  const last = partial.last_completed_post_id ? ` · last: #${esc(partial.last_completed_post_id)}` : '';
  jobStatus.innerHTML = `<span>${esc(name)}: ${esc(phase)} · ${current.progress ?? 0}% · ${processed}/${total}${elapsed}${active}${last}${stopping ? ' · stopping after current post' : ''}</span><button id="stopSetImport" class="status-stop" title="Stop set import" ${stopping ? 'disabled' : ''}><i data-lucide="square"></i></button>`;

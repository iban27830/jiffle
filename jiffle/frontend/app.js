      ...
      <div class="actions editor-state-actions"><button class="btn primary" id="analyzeCurrent"><i data-lucide="scan-line"></i>Analyze current</button><button class="btn" id="toggleBackgroundEditor" aria-expanded="${backgroundOpen}"><i data-lucide="image-plus"></i>Replace background</button><button class="btn danger" id="resetOriginal" ${state.is_edited?'':'disabled'}><i data-lucide="rotate-ccw"></i>Reset to original</button></div>
       <section id="backgroundPanel" class="background-panel" ${backgroundOpen?'':'hidden'}>
         ...
         <div id="backgroundAfterPreview" class="background-after-preview" ${savedPreview?.status==='ready'?'':'hidden'}>
           <details class="background-advanced-controls"><summary><span>Fine-tune mask edges</span><i data-lucide="chevron-down"></i></summary><div class="background-mask-controls"><label for="backgroundPreserve">Preserve soft details <output id="backgroundPreserveValue" for="backgroundPreserve">${savedPreserve}</output>%</label><input id="backgroundPreserve" type="range" min="0" max="100" value="${savedPreserve}"><small id="backgroundPreserveHint">Keeps more semi-transparent edge pixels. It cannot restore pixels removed completely by the model.</small><label for="backgroundRemoveHalo">Clean edge spill <output id="backgroundRemoveHaloValue" for="backgroundRemoveHalo">${savedRemoveHalo}</output>%</label><input id="backgroundRemoveHalo" type="range" min="0" max="100" value="${savedRemoveHalo}"><small id="backgroundRemoveHaloHint">Reduces weak translucent pixels around the subject. It does not rerun the model.</small></div></details>
...
async function showSettingsPage() {
  ...
  workspace.innerHTML = `<div class="settings-page"><form id="settingsForm" class="settings-form">
    ...
    <details class="settings-section"><summary><span class="section-icon"><i data-lucide="eraser"></i></span><span><strong>Background removal</strong><small>Automatic subject isolation and model access</small></span><i data-lucide="chevron-down"></i></summary><div class="settings-section-body">
      <div class="form-row"><label for="setting-background_model">Removal model</label><small class="field-hint">Automatic uses BiRefNet for fine details. RMBG-2.0 remains available for compatibility.</small><select class="control" id="setting-background_model" name="background_model"><option value="auto" ${data.background_model==='auto' || !data.background_model?'selected':''}>Automatic (recommended)</option><option value="birefnet" ${data.background_model==='birefnet'?'selected':''}>BiRefNet (fine detail)</option><option value="rmbg" ${data.background_model==='rmbg'?'selected':''}>RMBG-2.0 (compatibility)</option></select></div>

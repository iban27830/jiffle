...
async function showEditor() {
  ...
  const openMediaEditor = async mediaId => {
    ...
    const clampRemoveHalo=value=>{const numeric=Number(value);return Number.isFinite(numeric)?Math.max(0,Math.min(100,numeric)):0};
    ...
    const savedRemoveHalo=clampRemoveHalo(backgroundStateMatches?editorState.backgroundRemoveHalo ?? savedPreview?.remove_halo ?? 0:savedPreview?.remove_halo || 0);
      ...
      <div class="actions editor-state-actions"><button class="btn primary" id="analyzeCurrent"><i data-lucide="scan-line"></i>Analyze current</button><button class="btn" id="toggleBackgroundEditor" aria-expanded="${backgroundOpen}"><i data-lucide="image-plus"></i>Replace background</button><button class="btn danger" id="resetOriginal" ${state.is_edited?'':'disabled'}><i data-lucide="rotate-ccw"></i>Reset to original</button></div>
       <section id="backgroundPanel" class="background-panel" ${backgroundOpen?'':'hidden'}>
         ...
         <div id="backgroundAfterPreview" class="background-after-preview" ${savedPreview?.status==='ready'?'':'hidden'}>
           <div class="background-mask-controls"><label for="backgroundPreserve">Preserve details and shadows <output id="backgroundPreserveValue" for="backgroundPreserve">${savedPreserve}</output>%</label><input id="backgroundPreserve" type="range" min="0" max="100" value="${savedPreserve}"><small id="backgroundPreserveHint">Changes the transparency of soft edge pixels in the existing mask. It does not run RMBG-2.0 again.</small><label for="backgroundRemoveHalo">Remove background spill <output id="backgroundRemoveHaloValue" for="backgroundRemoveHalo">${savedRemoveHalo}</output>%</label><input id="backgroundRemoveHalo" type="range" min="0" max="100" value="${savedRemoveHalo}"><small id="backgroundRemoveHaloHint">Removes weak translucent halo pixels around the subject. It does not run RMBG-2.0 again.</small></div>
    ...
    const removeHalo=document.querySelector('#backgroundRemoveHalo');
    const removeHaloValueOutput=document.querySelector('#backgroundRemoveHaloValue');
    const removeHaloHint=document.querySelector('#backgroundRemoveHaloHint');
    ...
    const renderLocalPreview=async()=>{if(!preview||!foregroundImage)return;const preserveValue=Number(preserve.value);const removeHaloValue=Number(removeHalo.value);const dimensions=drawForegroundPreview(compositionForegroundCanvas.getContext('2d'),foregroundImage,preserveValue,preview.width,preview.height,removeHaloValue,'cutout');drawForegroundPreview(foregroundCanvas.getContext('2d'),foregroundImage,preserveValue,preview.width,preview.height,removeHaloValue,previewMode);if(dimensions?.hasSoftEdges){preserveHint.textContent='Soft edge pixels detected. Higher values keep more particles, shadows, and translucent details.';preserveHint.className=''}else{preserveHint.textContent='This removal has a hard mask with no semi-transparent edge pixels. Preserve cannot restore pixels already removed completely.';preserveHint.className='warning'}removeHaloHint.textContent=removeHaloValue?'Weak translucent halo pixels are being reduced before composition.':'Raises the threshold for weak translucent halo pixels around the subject.';if(selectedBackgroundData){if(!selectedBackgroundImage||selectedBackgroundImage.src!==selectedBackgroundData.content_url)selectedBackgroundImage=await loadImage(selectedBackgroundData.content_url);drawCompositionPreview(compositionResult.getContext('2d'),compositionForegroundCanvas,selectedBackgroundImage,Number(blur.value));compositionFigure.hidden=false;compositionReady=true}else{compositionFigure.hidden=true;compositionReady=false}applyZoom();updateApply();setBackgroundStatus(selectedBackgroundData?'Composition preview ready':'Preview ready · '+Number(preview.subject_coverage).toFixed(1)+'% subject','good');return dimensions};
    ...
    document.querySelector('#removeBackgroundPreview').onclick=async()=>{const button=document.querySelector('#removeBackgroundPreview');const previousPreview=preview;button.disabled=true;setBackgroundStatus(previousPreview?'Regenerating model mask...':'Preparing RMBG-2.0 and removing background...');try{preview=await api(`/api/v1/media/${mediaId}/background-preview`,{method:'POST',body:JSON.stringify({force:Boolean(previousPreview)})});button.innerHTML='<i data-lucide="scan-eye"></i>Regenerate model mask';const sameAsPrevious=Boolean(preview.same_as_previous);foregroundImage=null;selectedBackgroundImage=null;compositionReady=false;await resolveSelectedBackground().catch(()=>{});await prepareForeground();if(sameAsPrevious)setBackgroundStatus('Model returned the same mask. Adjust the controls below to change the result.','warn')}catch(error){if(error.message&&preview!==previousPreview){setBackgroundStatus(error.message,'error');afterPreview.hidden=false;updateApply();}else{preview=previousPreview;if(previousPreview){afterPreview.hidden=false;setBackgroundStatus(error.message,'error');updateApply()}else{afterPreview.hidden=true;setBackgroundStatus(error.message,'error')}}}finally{button.disabled=false;icons()}};
    ...
    removeHalo.oninput=()=>{removeHaloValueOutput.value=removeHalo.value;saveViewState('editor',{backgroundRemoveHalo:Number(removeHalo.value)});scheduleLocalPreview()};
...
async function showSettingsPage() {
  ...
  workspace.innerHTML = `<div class="settings-page"><form id="settingsForm" class="settings-form">
    ...
    <details class="settings-section"><summary><span class="section-icon"><i data-lucide="eraser"></i></span><span><strong>Background removal</strong><small>Hugging Face access for RMBG-2.0</small></span><i data-lucide="chevron-down"></i></summary><div class="settings-section-body">
      ${field('huggingface_token','Hugging Face token',{type:'password',secret:true,hint:'A read token is required to download RMBG-2.0. You can test a new token before saving it.'})}
      <small id="huggingFaceStatus" class="field-hint" aria-live="polite">${data.huggingface_token_configured?'A token is saved.':'No token is saved.'}</small>
      <div class="section-actions"><button id="testHuggingFace" type="button" class="btn" ${data.huggingface_token_configured?'':'disabled title="Save a token before testing access"'}><i data-lucide="plug-zap"></i>Test access</button><a class="btn" href="https://huggingface.co/settings/tokens" target="_blank" rel="noopener"><i data-lucide="external-link"></i>Manage tokens</a></div>
    </div></details>
  ...
  document.querySelector('#settingsForm').onsubmit = async event => {
    ...
    try { await Promise.all([api('/api/v1/settings',{method:'PATCH',body:JSON.stringify(payload)}),api('/api/v1/tag-rules',{method:'PUT',body:JSON.stringify({preferred:lines('preferred_tags'),blocked:lines('blocked_tags'),aliases})})]); if(payload.huggingface_token){data.huggingface_token_configured=true;const testButton=document.querySelector('#testHuggingFace');testButton.disabled=false;testButton.removeAttribute('title');document.querySelector('#huggingFaceStatus').textContent='A token is saved.';} toast('Settings saved'); document.querySelector('#settingsState').textContent='Settings saved'; }
    catch(error) { toast(error.message,true); }
  };
  ...
  const updateHuggingFaceTestButton=()=>{huggingFaceTestButton.disabled=!data.huggingface_token_configured&&!huggingFaceInput.value.trim();};
